...
const TeacherAssessmentsScreen: React.FC<TeacherAssessmentsScreenProps> = ({
    ...
    // Two-level navigation: subject grid → subject assessments
    ...
    useEffect(() => {
        ...
        const fetchSubjects = async () => {
            ...
            try {
                const res = await fetch(`/api/teacher/subjects?email=${encodeURIComponent(teacher.Email)}`);
                const data = await res.json();
                if (res.ok) setSubjects(data.subjects || []);
            } catch {
                // subjects fail silently — assessments still usable
            } finally {
    ...
    // Fetch assessments when a subject is active
    useEffect(() => {
        if (!teacher?.Email || !activeSubject) return;
        fetchAssessments();
    }, [teacher?.Email, activeSubject]);

    const fetchAssessments = async () => {
        if (!teacher?.Email || !activeSubject) return;
        setIsLoading(true);
        try {
            const url = `/api/teacher/assessments?email=${encodeURIComponent(teacher.Email)}&subject=${encodeURIComponent(activeSubject)}`;
            const response = await fetch(url);
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Failed to fetch assessments');
    ...
    const handleSelectSubject = (subject: TeacherSubject) => {
        setActiveSubject(subject.subject_name);
        setActiveGrade(subject.grade);
        setAssessments([]);
        setError(null);
    };
    ...
    const handleEdit = (assessment: Assessment) => {
        ...
        setFormData({
            ...
            grade: assessment.grade ?? activeGrade ?? 10
        });
        setShowModal(true);
    };
    ...
    const handleDelete = async (id: string) => {
        try {
            const response = await fetch(`/api/teacher/assessments?id=${id}`, { method: 'DELETE' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            const payload = { ...formData, teacher_email: teacher?.Email };
    ...
    // Deduplicate subjects by name (teacher may teach same subject to multiple grades)
    const uniqueSubjects = subjects.reduce<TeacherSubject[]>((acc, s) => {
    ...
    return (
        <div className="h-full bg-violet-950 overflow-y-auto relative">
            {/* Background blobs */}
            <div className="absolute top-[-10%] right-[-10%] w-64 h-64 bg-violet-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob pointer-events-none" />
            <div className="absolute bottom-[-10%] left-[-10%] w-64 h-64 bg-indigo-800 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000 pointer-events-none" />
            ...
            <div className="px-6 py-6 pb-32 relative z-10">
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-violet-200 text-sm">Select a subject to manage</p>
                            </div>
                        ...
                        {subjectsLoading ? (
                            <div className="grid grid-cols-2 gap-3">
                                {[1, 2, 3, 4].map(i => (
                                    <div key={i} className="bg-white/10 rounded-2xl p-5 animate-pulse h-28" />
                                ))}
                            </div>
                        ) : uniqueSubjects.length === 0 ? (
                            <div className="bg-white rounded-2xl p-8 text-center">
                                <span className="material-symbols-outlined text-5xl text-gray-300 mb-3 block">school</span>
                                <p className="text-gray-700 font-semibold">No subjects assigned</p>
                                <p className="text-sm text-gray-400 mt-1">Contact admin to set up your subjects</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-2 gap-3">
                                {uniqueSubjects.map((subject) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="bg-white rounded-2xl p-4 text-left shadow-sm hover:shadow-md transition-all active:scale-95 flex flex-col gap-3"
                                        >
                                            <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-sm`}>
                                                <span className="material-symbols-outlined text-white text-xl">{icon}</span>
                                            </div>
                                            <div>
                                                <p className="font-semibold text-gray-900 text-sm leading-tight">{subject.subject_name}</p>
                                                <p className="text-xs text-gray-400 mt-0.5">Grade {subject.grade}</p>
                                            </div>
                                        </button>
                                    );
                                })}
                ...
                {/* ── SUBJECT ASSESSMENTS VIEW ── */}
                {activeSubject && (
                    <>
                        {/* Header */}
                        <div className="flex items-center gap-4 mb-6">
                            <button
                                onClick={() => { setActiveSubject(null); setAssessments([]); setError(null); }}
                                className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors"
                            >
                                <span className="material-symbols-outlined">arrow_back</span>
                            </button>
                            <div className="flex-1">
                                ...
                                <p className="text-violet-200 text-sm">
                                    {activeGrade ? `Grade ${activeGrade}` : 'Assessments'}
                                </p>
                            </div>
                            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${getSubjectGradient(activeSubject)} flex items-center justify-center shadow-lg`}>
                                <span className="material-symbols-outlined text-white">{getSubjectIcon(activeSubject)}</span>
                            </div>
                        </div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="bg-red-50 border border-red-100 rounded-2xl p-4 text-red-600 flex items-center gap-2">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
                    <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            ...
                            {/* Subject badge — shown instead of the text input */}
                            {activeSubject && (
                                <div className="flex items-center gap-2 mb-5">
                                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getSubjectGradient(activeSubject)} flex items-center justify-center`}>
                                        <span className="material-symbols-outlined text-white text-base">{getSubjectIcon(activeSubject)}</span>
                                    </div>
                                    <span className="text-sm font-semibold text-gray-700">{activeSubject}</span>
                                    {activeGrade && (
                                        <span className="text-xs bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-medium">Grade {activeGrade}</span>
                                    )}
                                </div>
                            )}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none"
                                        >
                                            <option value={0}>Trial Students</option>
